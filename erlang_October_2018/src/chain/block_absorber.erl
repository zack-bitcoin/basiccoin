	...
	true ->
		...
		true ->
			...
			normal -> 
			    ...
			    Keep = lists:filter(fun(T) -> not(tx_pool_feeder:is_in(T#signed.data, OldTxs)) end, Txs),%This n**2 algorithm is slow. We can make it n*log(n) by sorting both lists first, and then comparing them.
